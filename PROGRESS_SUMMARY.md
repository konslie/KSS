...
- KOSPI/KOSDAQ 지표는 KRX/pykrx 기준값이 yfinance보다 최신이면 KRX/pykrx 값을 우선한다.
...
- 메인 제목은 `KO_데일리브리핑(yy.mm.dd)` 형식으로 변경했다.
- 상단 archive 영역의 sticky 동작을 제거했다.
- 주요 지표 카드는 세련된 라인 스파크라인을 사용한다.
- 주요 지표 섹션은 한국 지표와 미국/글로벌 지표의 장 마감 기준일을 각각 표시한다.
- 포트폴리오 테이블은 가격, 등락, 7일 추이, 수급, 영향, 핵심 이슈, 한줄 요약 중심으로 재구성했다.
- 포트폴리오 현황 UI는 `최근 종가` 한 칸 안에 종가/등락률/등락폭을 묶고, 수급은 외인·기관의 전일/7일 합계를 같은 셀에서 비교하도록 구성했다.
- `뉴스 n / 공시 m` 카운트는 제거하고 뉴스 제목 기반 한줄 요약으로 대체했다.
...
- 섹터별 브리핑은 가격 등락 나열을 피하고, 뉴스·공시 이슈와 기관/외인 수급이 가격 흐름을 확인하는지 또는 엇갈리는지 설명한다.
